...
//함수(function) : 특정 기능을 하는 구문을 독립된 부품으로 만들어
//재사용하고자 할 때 사용하는 문법이다.
...
console.log(hap(10, 20)); //undefined(정의가 되어있지 않음)
hap(10, 20); //30
...
/*
  ...
  (2) 함수몸체 정의 방법
    x => {return x*x}  //single line block으로 {} 중괄호를 생략하지 않으면 
    return을 반드시 해준다.

    x => x*x   //함수 몸체가 한 줄의 구문이면 중괄호를 생략하면서 return도 생략가능하다.

    ( ) => {return {a:1}}; //객체리턴할 때는 중괄호 필수
    ( ) => ({a:1});  //객체를 반환할 때는 ( )소괄호를 사용한다.
    ( ) => { let x = 10          // multi line block 일 때는 중괄호 생략 할 수 없음
               return x*x};
...
2. 클로저는 outer(외부)함수보다 중첩함수가 더 오래 유지되는 경우 중첩함수는
  이미 생명주기가 종료한 외부함수의 변수를 참조할 수 있다. 
...
// spread operator를 고정 파라미터와 같이 사용할 때는 맨 마지막에 한번 만 사용한다.
