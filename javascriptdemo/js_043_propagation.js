...
document.querySelector('.red').onclick = (e) => {
  alert('red');
  e.stopPropagation(); // 부모, 자식간에 이벤트 전파를 차단하는 메소드(stopPropagation)
};
...
document.querySelector('a').onclick = (e) => {
  alert('안녕하세요'); //'안녕하세요' 알림 뜬 후 페이지 이동
  //e.preventDefault(); // '안녕하세요' 알림 뜬 후 페이지 이동하지 않음(1)
  return false; // '안녕하세요' 알림 뜬 후 페이지 이동하지 않음(2)
};
